...
export function parseGoogleGeoCodeToAddressDetails(googleGeoCode: GoogleGeoCodeResponse): AddressDetails {
    ...
    if (city === undefined) {
        throw new MissingAddressDetailsError([AddressType.neighborhood, AddressType.political, AddressType.sublocality], `Missing  ${AddressType.sublocality}, ${AddressType.neighborhood} and ${AddressType.sublocality}types in address components`)
    }
    if (street === undefined) {
    ...
    if (streetNumber === undefined) {
        throw new MissingAddressDetailsError([GeocodingAddressComponentType.street_number], `Missing ${GeocodingAddressComponentType.street_number} type in address components`)
    }
    if (zip === undefined) {
...
export async function getAddressDetails(addressText: string, options: Options): Promise<AddressDetails> {
    const streetNumbersRegex = /^([0-9]+)(\s)([0-9]+ )/;
    let modifiedAddressText = addressText.replace(streetNumbersRegex, "$1-$3");
    let googleResponse = await getGoogleGeoCode(modifiedAddressText, options);
    try {
        return parseGoogleGeoCodeToAddressDetails(googleResponse);
    }
    catch (e) {
        if (e instanceof MissingAddressDetailsError) {
            if (e.missingTypes.includes(AddressType.administrative_area_level_2)) {
                const streetNumberRegex = /^[0-9]+\s|^[0-9]+-[0-9]+\s/;
                modifiedAddressText = modifiedAddressText.replace(streetNumberRegex, '');
                googleResponse = await getGoogleGeoCode(modifiedAddressText, options);
                return parseGoogleGeoCodeToAddressDetails(googleResponse);
            } else {
                throw e;
            }
